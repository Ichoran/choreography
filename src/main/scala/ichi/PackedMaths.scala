...
object PackedMaths {
  implicit final class ByteBits(val repr: Byte) extends AnyVal {
    def bit0 = (repr&0x01)
    def bit0(n: Int) = new ByteBits(((repr&0xFE)|(n&0x01)).toByte)
    def bit1 = (repr&0x02)
    def bit1(n: Int) = new ByteBits(((repr&0xFD)|(n&0x02)).toByte)
    def bit2 = (repr&0x04)
    def bit2(n: Int) = new ByteBits(((repr&0xFB)|(n&0x04)).toByte)
    def bit3 = (repr&0x08)
    def bit3(n: Int) = new ByteBits(((repr&0xF7)|(n&0x08)).toByte)
    def bit4 = (repr&0x10)
    def bit4(n: Int) = new ByteBits(((repr&0xEF)|(n&0x10)).toByte)
    def bit5 = (repr&0x20)
    def bit5(n: Int) = new ByteBits(((repr&0xDF)|(n&0x20)).toByte)
    def bit6 = (repr&0x40)
    def bit6(n: Int) = new ByteBits(((repr&0xBF)|(n&0x40)).toByte)
    def bit7 = (repr&0x80)
    def bit7(n: Int) = new ByteBits(((repr&0x7F)|(n&0x80)).toByte)
  }
