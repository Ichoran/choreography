...
package object maths {
  ...
  // Missing math
  @inline final def log2(f: Float) = (log(f)*LnTwo).toFloat
  @inline final def log2(d: Double) = log(d)*LnTwo
  ...
  object StdDev {
    ...
    def weighted[@specialized(Float,Double) A](v: Array[A], w: Array[A])(implicit f: Fn1D[A]): StdDev = {
      var mean, sw, ssw = 0.0
      var i = 0
      while (i < v.length) { val wi = f(w(i)); mean += f(v(i))*wi; sw += wi; ssw += wi*wi; i += 1 }
      val isw = 1.0/sw
      val pvf = sw/(sw*sw - ssw)   // Correction factor for sample deviation
      mean *= isw
      // Now need to find a sensible approximation for SD
      var sse = 0.0
      val maxent = -log2(1.0/v.length)
      var ent = 0.0
      i = 0
      while (i < v.length) { val d = f(v(i))-mean; val wi = f(w(i))*isw; sse += wi*d*d; ent -= wi*log2(wi); i += 1 }
      val n = math.max(math.max(1,math.min(2,v.length)), rint(v.length*min(1,ent/maxent)).toInt)
      StdDev(n, mean, sqrt(max(1,n)*sse/max(1,n-1)))
    }
  ...
  case class DevDev(n: Int, mean: Double, override val sem: Double, subsem: Double, gsd: Double) extends StandardDeviation {
    lazy val sd = sem*sqrt(max(1,n-1))
  }
  object DevDev {
    def apply(vs: Seq[StandardDeviation]): DevDev = if (vs.isEmpty) new DevDev(0, Double.NaN, Double.NaN, Double.NaN, 0) else {
      val N = vs.length
      if (N==1) { val sd = vs.head; return new DevDev(sd.n, sd.mean, sd.sem, 0, 0) }  // Short-circuit for trivial case
      val ns = new Array[Int](N)
      val means = new Array[Double](N)
      val sems = new Array[Double](N)
      val weights = new Array[Double](N)
      val sw = 0.0
      var i = 0
      vs.foreach{ v => sems(i) = v.sem; means(i) = v.mean; ns(i) = v.n; i += 1 }
      
      // First we need an estimate of variability across the entire data set
      var cuml = 0.0
      var sn = 0L
      i = 0
      while (i < N) {
        sn += ns(i)
        val s = sems(i)
        val sd = s*sqrt(max(1,ns(i)-1))
        cuml += sd*sd*max(1,ns(i)-1)
        i += 1
      }
      val gsd = sqrt(cuml/max(1,sn-N))
      // Now we find tentative weights
      i = 0
      cuml = 0.0
      var wssq = 0.0
      var esem = 0.0
      while (i < N) {
        val sqei = sems(i)*sems(i)
        val sqej = gsd*gsd/max(1,ns(i)-1)
        val w = 1.0/max(sqei,sqej)
        weights(i) = w
        cuml += w
        wssq += w*w
        esem += sqej*w
      ...
      val prestats = StdDev.weighted(means, weights)
      //return new DevDev(prestats.n, prestats.mean, prestats.sem, esem, gsd)
      // And figure out how much more deviation we observed than expected
      val expssq = cuml/(cuml*cuml - wssq)
      val actssq = prestats.sumOfSquares
      val excess = sqrt(max(0.0,actssq-expssq)/(N-1))
      // If there is excess deviation, our original measurements are perturbed so we need to do them again
      val stats = if (excess <= 0) prestats else {
        i = 0
        cuml = 0
        wssq = 0
        esem = 0
        while (i < N) {
          val sqei = sems(i)*sems(i)
          val sqej = gsd*gsd/max(1,ns(i)-1)
          val sqe = max(sqei, sqej) + excess*excess
          val w = 1.0/sqe
          weights(i) = w
          cuml += w
          wssq += w*w
        ...
        esem = sqrt(esem/cuml)
        StdDev.weighted(means, weights)
      }
      // Finally, adjust sem to be a plausible compromise between individual and group measurements
      val essq = cuml/(cuml*cuml - wssq)
      val assq = stats.sumOfSquares
      val sem = stats.sem*sqrt(1+essq/assq)
      new DevDev(stats.n, stats.mean, sem, esem, gsd)
    }
    def apply[@specialized (Float, Double)A](vs: Seq[Array[A]])(implicit f: Fn1D[A]): DevDev = DevDev.apply(vs.map(a => StdDev(a)(f)))
  }
