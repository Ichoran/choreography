...
object MwtWconConvert {
  ...
  def convertOldToWcon(summary: Option[Array[Byte]], blobs: Array[Array[Byte]], file: File, base: String, prefix: String) {
    ...
    val ans = wcon.setUnits().setFile(new File(file.getParent, id + ".wcon"))
  ...
  def convertDirectoryToWcon(dir: File) {
  ...
  def convertWconToZippedOld(wcon: File) {
    ???
  }

  def convertZippedOldToWcon(zip: File) {
  ...
  def main(args: Array[String]) {
    ...
    val files = args.map{ a =>
      ...
      if (fa.isDirectory && fa.listFiles.exists(_.getName.endsWith("blobs"))) fa
        ...
        if (fa.getName.endsWith("wcon")) fa
        else if (fa.getName.endsWith("wcon.zip")) throw new Exception(f"Zipped WCON files not supported: ${fa.getCanonicalFile.getPath}")
        else if (fa.getName.endsWith(".zip")) fa
    ...
    files.foreach{ fa =>
      if (fa.isDirectory) convertDirectoryToWcon(fa)
      else if (fa.getName.endsWith("wcon")) convertWconToZippedOld(fa)
      else convertZippedOldToWcon(fa)
    }
  }
}
